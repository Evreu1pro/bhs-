...
export function renderHeroProcess() {
	...
	const layers = stages.map((s, i) => { const vid = s.video ? `<video class="hero__video" data-video muted playsinline preload="metadata" poster="${assets.heroPosters[s.poster]}"><source src="${assets.heroVideos[s.video]}" type="video/mp4" /></video>` : ""; return `<div class="hero__layer${i === 0 ? " is-active" : ""}" data-layer="${i}">${vid}<div class="hero__vfallback" aria-hidden="true"></div></div>` }).join("")
...
export function initHeroProcess() {
	...
	const setActive = (idx) => {
		...
		// Аннотации — только на 4-м этапе (Recycling, idx === 3)
		if (idx === 3) annots.removeAttribute("hidden")
		else annots.setAttribute("hidden", "")
		const active = layers[idx] && layers[idx].querySelector("video")
		if (active && active.readyState >= 2 && active.paused) active.play().catch(() => {})
	}
	if (prefersReducedMotion) {
		hero.classList.add("is-static")
		setActive(0)
		stageTexts.forEach((l) => l.classList.add("is-active"))
		if (annots) annots.removeAttribute("hidden")
	} else {
		const onScroll = () => {
	...
	annotEls.forEach((btn) => btn.addEventListener("click", (e) => { e.stopPropagation(); const h = hotspots.find((x) => x.id === btn.dataset.hotspot); if (window.innerWidth <= 860) { openPanel(h); return } const wasOpen = btn.classList.contains("is-open"); annotEls.forEach((b) => b.classList.remove("is-open")); if (!wasOpen) btn.classList.add("is-open") }))
